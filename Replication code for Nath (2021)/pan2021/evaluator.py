...
def compute_score_multiple_predictions(truth, solutions, key, labels, solutions_folder):
    ...
    print("You have a problem here:", [(list(truth.keys())[i], task2_truth[i], task2_solution[i]) for i,j in enumerate(task2_truth) if len(task2_truth[i]) != len(task2_solution[i])])
...
def main():
    ...
    task2_results = compute_score_multiple_predictions(truth, solutions, 'changes', labels=[0, 1], solutions_folder=args.predictions)
    task3_results = compute_score_multiple_predictions(truth, solutions, 'paragraph-authors', labels=[1,2,3,4,5], solutions_folder=args.predictions)
...
# evaluator.py -p "/home/sukanya/PhD/CLEF/2021/runs/run_15_apr_1/" -t "/home/sukanya/PhD/CLEF/2021/train/" -o output
